...
export async function loginUser(email, password) {
  ...
  console.log("Réponse normalisée:", normalized);

  return normalized;
}

function normalizeAuthResponse(data) {
  // Backend already returns the new role structure
  if (
    data?.user &&
    (data.user.globalRoles || data.user.departmentRoles)
  ) {
    return {
      token: data.token,
      user: data.user,
  ...
  const isGlobalAdmin = data?.role === "ADMIN";
  ...
  return {
    ...
    user: {
      ...
      globalRoles: isGlobalAdmin ? ["ADMIN"] : [],

      departmentRoles:
        !isGlobalAdmin && data?.role
          ? [
              {
                departmentId: null,
                departmentName: null,
...
export async function requestPasswordReset(email) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      errorMsg || "Impossible d'envoyer le code de vérification."
    );
  }
...
export async function resetPassword({ email, otp, newPassword }) {
  ...
  if (!res.ok) {
    ...
    throw new Error(
      errorMsg || "Impossible de réinitialiser le mot de passe."
    );
  }
...
// Active/désactive un compte. Séparé de updateUser pour rester explicite
// dans les logs et permettre plus tard une route backend dédiée avec ses
// propres règles (ex: notification email, révocation de session).
...
export async function fetchComments(taskId) {
  const response = await fetch(
    `${API_URL}/comments?taskId=${taskId}`,
    {
      headers: getAuthHeader(),
    }
  );
...
export async function fetchTimesheetEntries({
  ...
  const response = await fetch(
    `${API_URL}/timesheet-entries?${params}`,
    {
      headers: getAuthHeader(),
    }
  );
...
export async function fetchWeeklyReport({ userId, weekStart }) {
  const response = await fetch(
    `${API_URL}/weekly-reports?userId=${userId}&weekStart=${weekStart}`,
    {
      headers: getAuthHeader(),
    }
  );
...
export async function fetchPerformanceComments({
  userId,
  weekStart,
}) {
  const response = await fetch(
    `${API_URL}/performance-comments?userId=${userId}&weekStart=${weekStart}`,
    {
      headers: getAuthHeader(),
    }
  );
...
export async function createPerformanceComment({
  targetUserId,
  weekStart,
  content,
}) {
...
export async function fetchAllWeeklyReports(userId) {
  ...
  const response = await fetch(
    `${API_URL}/weekly-reports?userId=${userId}`,
    {
      headers: {
        Authorization: `Bearer ${currentUser?.token}`,
      },
